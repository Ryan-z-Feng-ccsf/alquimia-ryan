...
/* Private manifest types and helpers for parsing JSON mappings used by the
** ONNX Alquimia interface. Manifest-owned storage is released after mappings
...
bool OnnxAlquimiaLoadManifest(
    /* JSON file path */
    const char *path,
    /* Populates the manifest. On failure, resources are freed internally. 
    ** This can be assigned directly to an OnnxEngine's engine->manifest;
